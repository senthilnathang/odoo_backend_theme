odoo.define('web.ProfileView', function (require) {
"use strict";
 ...
 */
var ProfileView = View.extend(common.FieldManagerMixin, {
    /**
    ...
    display_name: _lt('Profile'),
    view_type: "profile",
    multi_record: false,
    accesskey: "J",
    icon: 'fa-desktop',
    /**
     * @constructs instance.web.ProfileView
    ...
    /**
     * Render the buttons according to the ProfileView.buttons template and add listeners on it.
     * Set this.$buttons with the produced jQuery element
     * @param {jQuery} [$node] a jQuery node where the rendered buttons should be inserted
     * $node may be undefined, in which case the ProfileView inserts them into this.options.$buttons
     * or into a div of its template
     */
    render_buttons: function($node) {
        this.$buttons = $(QWeb.render("ProfileView.buttons", {'widget': this}));
     ...
     * $node may be undefined, in which case the ProfileView inserts the sidebar in a
     * div of its template
     **/
    ...
    /**
     * Render the pager according to the ProfileView.pager template and add listeners on it.
     ...
     * $node may be undefined, in which case the ProfileView inserts the pager into this.options.$pager
     * or into a div of its template
     */
    render_pager: function($node) {
        if (this.options.pager) {
            ...
            this.$pager = $(QWeb.render("ProfileView.pager", {'widget': self}));
    ...
    on_button_delete: function() {
        ...
        var def = $.Deferred();
        this.has_been_loaded.done(function() {
            //~ if (self.datarecord.id && swal(_t("Really want to delete this record?"))) {
            if (self.datarecord.id && swal({
        title: "Are you sure?",
            ...
            $.when.apply($, deferred).always(function () {
                ...
                if (form_invalid) {
                    ...
                    if (!self.datarecord.id) {
                        ...
                        // Not dirty, noop save
                        //swal("Warning!", "No changes  has been made in the record.", "warning");
                        ...
                        save_deferral = self.dataset.write(self.datarecord.id, values, {readonly_fields: readonly_values}).then(function(r) {
                            ...
                            //toastr('sucess',"Hi, welcome to Inspinia. This is example of Toastr notification box.")
    ...
    on_invalid: function() {
        ...
        //warnings.unshift('<ul>');
        //warnings.push('</ul>');
        swal(_t("The following fields are invalid:"), warnings.join(','));
    },
    /**
    ...
    open_defaults_dialog: function () {
        ...
        d.template = 'ProfileView.set_default';
        d.open();
    },
...
core.view_registry.add('profile', ProfileView);
